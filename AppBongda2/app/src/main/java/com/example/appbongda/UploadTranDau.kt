...
import com.example.appbongda.databinding.ActivityUploadTranDauBinding
import com.google.firebase.database.FirebaseDatabase
import com.google.firebase.storage.FirebaseStorage
import java.text.DateFormat
import java.util.*

class UploadTranDau : AppCompatActivity() {
    private lateinit var binding: ActivityUploadTranDauBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityUploadTranDauBinding.inflate(layoutInflater)
        ...
        val activityResultLauncher = registerForActivityResult<Intent, ActivityResult>(
            ...
            if (result.resultCode == RESULT_OK) {
                ...
                Toast.makeText(this@UploadTranDau, "No Image Selected", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun saveData(){
        ...
        val builder = AlertDialog.Builder(this@UploadTranDau)
        ...
        storageReference.putFile(uri!!).addOnSuccessListener { taskSnapshot ->
            ...
            val builder2 = AlertDialog.Builder(this@UploadTranDau)
    ...
    private fun uploadData(){
        val title = binding.uploadteam1.text.toString()
        val desc = binding.uploadMota.text.toString()
        val priority = binding.uploateam2.text.toString()
        val dataClass = DataTranDau(title, desc, priority, imageURL,imageURL2)
        val currentDate = DateFormat.getDateTimeInstance().format(Calendar.getInstance().time)
        FirebaseDatabase.getInstance().getReference("trandau").child(currentDate)
            .setValue(dataClass).addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    Toast.makeText(this@UploadTranDau, "Saved", Toast.LENGTH_SHORT).show()
                    finish()
                }
            }.addOnFailureListener { e ->
                Toast.makeText(
                    this@UploadTranDau, e.message.toString(), Toast.LENGTH_SHORT).show()
            }
    }
}