...
class UploadActivity : AppCompatActivity() {
    ...
    private fun saveData(){
        ...
        storageReference.putFile(uri!!).addOnSuccessListener { taskSnapshot ->
            ...
            val storageReference2 = FirebaseStorage.getInstance().reference.child("Task Images2")
                .child(uri2!!.lastPathSegment!!)
            val builder2 = AlertDialog.Builder(this@UploadActivity)
    ...
    private fun uploadData(){
        ...
        val priority = binding.uploadPosition.text.toString()
        val dataClass = DataClass(title, desc, priority, imageURL,imageURL2)
        val currentDate = DateFormat.getDateTimeInstance().format(Calendar.getInstance().time)
        FirebaseDatabase.getInstance().getReference("appbongda").child(currentDate)
